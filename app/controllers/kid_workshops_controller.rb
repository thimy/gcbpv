class KidWorkshopsController < SecretariatController
  ...
  def show
    @confirmed_count = @kid_workshop.subscriptions.registered(@season).confirmed.has_workshop(@kid_workshop).size if @kid_workshop.present?
    @optional_count = @kid_workshop.subscriptions.registered(@season).optional.has_workshop(@kid_workshop).size + @kid_workshop.subscriptions.inquired(@season).has_workshop(@kid_workshop).size if @kid_workshop.present?
  end
  ...
  def create
    ...
    respond_to do |format|
      if @kid_workshop.save
        format.html { redirect_to kid_workshop_url(@kid_workshop), notice: "Workshop was successfully created." }
  ...
  def update
    respond_to do |format|
      if @kid_workshop.update(kid_workshop_params)
        format.html { redirect_to kid_workshop_url(@kid_workshop), notice: "L’atelier Enfance a bien été modifié." }
